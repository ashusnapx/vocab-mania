...
import { Menu, X, Sun, Moon, BookOpen, Star, BarChart3, Settings } from "lucide-react";
import { useTheme } from "next-themes";
import { APP_NAME } from "@/lib/constants";
import { useUser } from "@/lib/auth-context";

const APP_NAV = [
  { label: "Seekhna", href: "/learn", icon: BookOpen },
          ...
          {APP_NAV.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[13.5px] font-medium transition-all ${
                  isActive
              ...
              >
                {isSettings && user ? (
                  <div className="w-5 h-5 rounded-full overflow-hidden flex items-center justify-center bg-violet-primary/20 text-violet-primary dark:bg-[#60a5fa]/20 dark:text-[#60a5fa] border border-violet-primary/20 dark:border-[#60a5fa]/30 text-[9px] font-black shrink-0">
              ...
              {APP_NAV.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[14.5px] font-medium transition-colors ${
                      isActive
                        ? "bg-violet-primary/10 text-violet-primary"
                  ...
                  >
                    {isSettings && user ? (
                      <div className="w-6 h-6 rounded-full overflow-hidden flex items-center justify-center bg-violet-primary/20 text-violet-primary dark:bg-[#60a5fa]/20 dark:text-[#60a5fa] border border-violet-primary/20 dark:border-[#60a5fa]/30 text-[10px] font-black shrink-0">
                        {user.user_metadata?.avatar_url ? (
                          <img
                            src={user.user_metadata.avatar_url}
                            alt="Profile"
