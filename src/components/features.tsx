...
const iconPaths: Record<string, string> = {
  schedule: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z",
  ...
  quiz: "M21 5c-1.11-.35-2.33-.5-3.5-.5-1.95 0-4.05.4-5.5 1.5-1.45-1.1-3.55-1.5-5.5-1.5S2.45 4.9 1 6v14.65c0 .25.25.5.5.5.1 0 .15-.05.25-.05C3.1 20.45 5.05 20 6.5 20c1.95 0 4.05.4 5.5 1.5 1.35-.85 3.8-1.5 5.5-1.5 1.65 0 3.35.3 4.75 1.05.1.05.15.05.25.05.25 0 .5-.25.5-.5V6c-.6-.45-1.25-.75-2-1zm0 13.5c-1.1-.35-2.3-.5-3.5-.5-1.7 0-4.15.65-5.5 1.5V8c1.35-.85 3.8-1.5 5.5-1.5 1.2 0 2.4.15 3.5.5v11.5z",
...
export function Features() {
  ...
  return (
    <section id="features" className="bg-surface-container py-16 md:py-24">
      <div ref={ref} className="section-wrap">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.items.map((feature, i) => (
            <div
              key={feature.title}
              className={`group relative overflow-hidden rounded-[24px] p-6 transition-all duration-300 ease-[cubic-bezier(0.2,0,0,1)] hover:shadow-md hover:scale-[1.02] ${
            ...
            >
              ...
              <div className="pointer-events-none absolute inset-0 rounded-[24px] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
                style={{ boxShadow: "inset 0 0 40px rgba(37, 99, 235, 0.06)" }}
              />

              {/* Glow reveal */}
              <div className="pointer-events-none absolute -top-14 -right-14 h-36 w-36 rounded-full bg-primary/0 opacity-0 blur-2xl transition-all duration-500 group-hover:bg-primary/10 group-hover:opacity-100" />

              <div className="relative">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-[16px] bg-secondary-container transition-all duration-300 group-hover:bg-primary-container group-hover:scale-110 group-hover:rotate-3">
                  <svg className="h-6 w-6 text-on-secondary-container transition-colors duration-300 group-hover:text-on-primary-container" viewBox="0 0 24 24" fill="currentColor">
                    <path d={iconPaths[feature.icon] || ""} />
                  </svg>
                </div>
                <span className="text-[12px] font-medium uppercase tracking-[0.06em] text-primary">
                  {feature.category}
                </span>
                <h3 className="mt-2 text-[20px] font-medium text-on-surface">
                  {feature.title}
                </h3>
                <p className="mt-2 text-[15px] leading-relaxed text-outline">
                  {feature.description}
                </p>
              </div>
            </div>
